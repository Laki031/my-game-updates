...
WHITE = (255, 255, 255)
BLACK = (0, 0, 0)
RED = (255, 100, 100)  # Яркий красный с теплым оттенком
BLUE = (100, 100, 255)  # Яркий синий с легким фиолетовым
GRAY = (200, 200, 220)  # Нейтральный серо-голубой
DARK_GRAY = (80, 80, 100)
LIGHT_GRAY = (230, 230, 250)
BROWN = (140, 90, 60)  # Темный теплый коричневый
PURPLE = (200, 120, 200)  # Яркий фиолетовый
ORANGE = (255, 180, 100)  # Яркий оранжевый
CYAN = (100, 220, 220)  # Нежный голубой
DARK_BLUE = (30, 30, 60)
YELLOW = (255, 240, 120)  # Яркий теплый желтый
GOLD = (255, 200, 0)
PINK = (255, 160, 160)
...
button_font = pygame.font.Font(None, 28)  # Увеличен шрифт для кнопок
...
class Territory:
    def __init__(self, x, y):
        ...
        self.population = 0
        self.level = 1
        self.buildings = []  # Список построек (до 2)
...
BUFFS_DEBUFFS = {
    "resource_bonus": {"name": "Ресурсный бонус", "effect": lambda t: min(t.resources * 1.5, GAME_CONFIG['MAX_TROOPS']),
                       "color": GOLD},
    "troop_bonus": {"name": "Бонус войск",
                    "effect": lambda t: min(sum(t.troops.values()) * 1.2, GAME_CONFIG['MAX_TROOPS']), "color": YELLOW},
    "defense_bonus": {"name": "Бонус защиты",
                      "effect": lambda t: min(sum(t.troops.values()) * 1.3 if t.owner else sum(t.troops.values()),
                                              GAME_CONFIG['MAX_TROOPS']), "color": CYAN},
...
def generate_map(seed):
    ...
    # Проверка, является ли сид именем файла .txt
    if seed.endswith('.txt') and os.path.exists(seed):
        load_map_from_txt(seed)
    else:
        random.seed(int(hashlib.sha256(seed.encode()).hexdigest(), 16))
        for x in range(grid_width):
            for y in range(grid_height):
                if random.random() < GAME_CONFIG['WALL_DENSITY'] and (x, y) not in [(0, 0),
    ...
    for x in range(grid_width):
        for y in range(grid_height):
            ...
            if territories[x][y].buff_debuff and territories[x][y].effect_timer > 0:
                ...
                pygame.draw.circle(effect_surface, (*effect_color, alpha), (rect.width // 2, rect.height // 2),
                                   rect.width // 2)
                screen.blit(effect_surface, rect.topleft)
                territories[x][y].effect_timer -= 1

            # Только количество войск
...
def draw_ui():
    ...
    if selected:
        ...
        buttons = [
            {"text": "Пехота (1)", "cost_res": GAME_CONFIG['ARMY_COST']['Infantry']['resources'],
             "cost_pop": GAME_CONFIG['ARMY_COST']['Infantry']['population'],
             "action": lambda: create_army(x, y, 'Infantry')},
            {"text": "Кавалерия (2)", "cost_res": GAME_CONFIG['ARMY_COST']['Cavalry']['resources'],
             "cost_pop": GAME_CONFIG['ARMY_COST']['Cavalry']['population'],
             "action": lambda: create_army(x, y, 'Cavalry')},
            {"text": "Лучники (3)", "cost_res": GAME_CONFIG['ARMY_COST']['Archers']['resources'],
             "cost_pop": GAME_CONFIG['ARMY_COST']['Archers']['population'],
             "action": lambda: create_army(x, y, 'Archers')},
            {"text": "Прокачать (4)", "cost_res": GAME_CONFIG['UPGRADE_COST']['resources'],
             "cost_pop": GAME_CONFIG['UPGRADE_COST']['population'], "action": lambda: upgrade_territory(x, y)},
            {"text": "Форт (5)", "cost_res": GAME_CONFIG['FORT_COST']['resources'],
             "cost_pop": GAME_CONFIG['FORT_COST']['population'], "action": lambda: build_structure(x, y, 'fort')},
            {"text": "Город (6)", "cost_res": GAME_CONFIG['CITY_COST']['resources'],
             "cost_pop": GAME_CONFIG['CITY_COST']['population'], "action": lambda: build_structure(x, y, 'city')},
            {"text": "Башня (7)", "cost_res": GAME_CONFIG['TOWER_COST']['resources'],
             "cost_pop": GAME_CONFIG['TOWER_COST']['population'], "action": lambda: build_structure(x, y, 'tower')},
        ]
        y_offset += 10
        mouse_pos = pygame.mouse.get_pos()
        hover_button = None
        for i, btn in enumerate(buttons):
            col = i % 2
            row = i // 2
            btn_rect = pygame.Rect(grid_width * GAME_CONFIG['GRID_SIZE'] + 10 + col * (ui_width // 2 - 10),
                                   y_offset + row * 45, ui_width // 2 - 20, 40)
            ...
            text_surface = button_font.render(f"{btn['text']} - {btn['cost_res']}р/{btn['cost_pop']}н", True,
                                              WHITE)  # Белый текст
    ...
    if event_message and event_timer > 0:
        ...
        event_surface.blit(event_text, (
        event_surface.get_width() // 2 - event_text.get_width() // 2, 50 - event_text.get_height() // 2))
...
def simple_battle(attacker_troops, defender_troops, defender_buildings):
    atk_total = sum(attacker_troops.values())
...
def move_troops(from_x, from_y, to_x, to_y):
    if (abs(from_x - to_x) + abs(from_y - to_y) == 1 and
        ...
        if total_target + sum(territories[from_x][from_y].troops.values()) <= GAME_CONFIG['MAX_TROOPS']:
            territories[to_x][to_y].troops = {
                k: territories[to_x][to_y].troops[k] + territories[from_x][from_y].troops[k]
...
def attack(from_x, from_y, to_x, to_y, is_player=True):
    ...
    attack_limit = (
        GAME_CONFIG['ATTACKS_PER_TURN_LIMIT'] + 2 if territories[from_x][from_y].buff_debuff == "speed_boost" else
        GAME_CONFIG['ATTACKS_PER_TURN_LIMIT'])
    if (abs(from_x - to_x) + abs(from_y - to_y) == 1 and
        ...
        if (resources >= GAME_CONFIG['ATTACK_COST'] and
            ...
            success, remaining_troops = simple_battle(attacker_troops, defender_troops,
                                                      territories[to_x][to_y].buildings)
            if success:
                ...
                territories[to_x][to_y].buildings = []  # Постройки разрушаются при захвате
            else:
                territories[from_x][from_y].troops = {'Infantry': 0, 'Cavalry': 0, 'Archers': 0}
...
def add_attack(from_x, from_y, to_x, to_y):
    if attack(from_x, from_y, to_x, to_y):
        # Красная обводка только если это атака на чужую клетку
...
def create_army(x, y, army_type, is_player=True):
    ...
    if (territories[x][y].owner == (1 if is_player else 2) and
        ...
        if territories[x][y].buff_debuff:
            if territories[x][y].buff_debuff in ["resource_bonus", "troop_bonus", "defense_bonus", "resource_penalty",
                                                 "troop_penalty"]:
                ...
                territories[x][y].troops = {k: min(int(v * ratio), GAME_CONFIG['MAX_TROOPS']) for k, v in
                                            territories[x][y].troops.items()}
    ...
    if (territories[x][y].owner == 1 and
        ...
        if territories[x][y].buff_debuff:
            if territories[x][y].buff_debuff in ["resource_bonus", "resource_penalty"]:
                territories[x][y].resources = int(
                    BUFFS_DEBUFFS[territories[x][y].buff_debuff]["effect"](territories[x][y]))
        player_resources -= cost_res
...
def build_structure(x, y, structure_type):
    ...
    if (territories[x][y].owner == 1 and
            ...
            len(territories[x][y].buildings) < 2 and  # Максимум 2 постройки
...
def smart_bot_turn():
    ...
    bot_population += sum(
        GAME_CONFIG['POPULATION_GROWTH'] for row in territories for t in row if t.owner == 2 and not t.is_wall)
    ...
    for x in range(grid_width):
        for y in range(grid_height):
            if territories[x][y].owner == 2:
                total_troops = sum(territories[x][y].troops.values())
                is_border = any(
                    territories[nx][ny].owner == 1 for nx, ny in [(x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)]
                ...
                if (bot_resources >= min(c['resources'] for c in GAME_CONFIG['ARMY_COST'].values()) and
                    ...
                    bot_resources -= GAME_CONFIG['UPGRADE_COST']['resources']
                      ...
                      bot_resources >= GAME_CONFIG['CITY_COST']['resources'] and bot_population >=
                      GAME_CONFIG['CITY_COST']['population'] and
                    ...
                    territories[x][y].population -= GAME_CONFIG['CITY_COST']['population']
                      ...
                      bot_resources >= GAME_CONFIG['TOWER_COST']['resources'] and bot_population >=
                      GAME_CONFIG['TOWER_COST']['population'] and
                    ...
                    territories[x][y].population -= GAME_CONFIG['TOWER_COST']['population']
                      ...
                      bot_resources >= GAME_CONFIG['FORT_COST']['resources'] and bot_population >=
                      GAME_CONFIG['FORT_COST']['population'] and
    ...
    for x in range(grid_width):
        for y in range(grid_height):
            if territories[x][y].owner == 2 and sum(territories[x][y].troops.values()) > 1 and (
            x, y) not in bot_used_in_turn:
                    ...
                    if (0 <= nx < grid_width and 0 <= ny < grid_height and
                        ...
                        if atk_total > def_total * 1.2:
                            priority = (territories[nx][ny].resources * 2 +
                                        ...
                                        (3 if territories[nx][ny].buff_debuff in ["resource_bonus",
                                                                                  "troop_bonus"] else 0))
                            attack_options.append((x, y, nx, ny, priority))
    attack_options.sort(key=lambda x: x[4], reverse=True)
    attack_limit = (GAME_CONFIG['ATTACKS_PER_TURN_LIMIT'] + 2 if any(
        t.buff_debuff == "speed_boost" for row in territories for t in row if t.owner == 2) else GAME_CONFIG[
        'ATTACKS_PER_TURN_LIMIT'])
...
def collect_resources():
    ...
    player_resources += sum(t.resources + (GAME_CONFIG['CITY_RESOURCE_BONUS'] if 'city' in t.buildings else 0)
                            for row in territories for t in row if t.owner == 1 and not t.is_wall)
    player_population += sum(
        GAME_CONFIG['POPULATION_GROWTH'] for row in territories for t in row if t.owner == 1 and not t.is_wall)
    for row in territories:
        for t in row:
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT or (event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE):
                ...
                if event.key == pygame.K_RETURN and not settings_input:
                    ...
                    if event.key == pygame.K_RETURN and seed_input:
                        try:
                            ...
                            elif settings_input in ['FORT_COST_resources', 'FORT_COST_population',
                                                    'CITY_COST_resources', 'CITY_COST_population',
                                                    'TOWER_COST_resources', 'TOWER_COST_population',
                                                    'UPGRADE_COST_resources']:
                                key, res_type = settings_input.rsplit('_', 1)
                                GAME_CONFIG[key][res_type] = int(value)
                            else:
                                GAME_CONFIG[
                                    settings_input] = value if 'INTELLIGENCE' in settings_input or 'CHANCE' in settings_input or 'DENSITY' in settings_input else int(
                                    value)
                        except ValueError:
                            pass
                        seed_input = ""
            ...
            elif event.button == 1 and hover_button:
                hover_button()
    ...
    if game_state == MENU:
        ...
        screen.fill(BLACK)
        prompt = font.render("Введите сид или имя файла .txt и нажмите ENTER:", True, WHITE)
        seed_text = font.render(seed_input, True, WHITE)
        screen.blit(prompt, (GAME_CONFIG['WIDTH'] // 2 - prompt.get_width() // 2, GAME_CONFIG['HEIGHT'] // 3 - 50))
        screen.blit(seed_text, (GAME_CONFIG['WIDTH'] // 2 - seed_text.get_width() // 2, GAME_CONFIG['HEIGHT'] // 3))
        if preview_map:
